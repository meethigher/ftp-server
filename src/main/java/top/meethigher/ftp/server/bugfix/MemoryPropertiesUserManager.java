 ...
 * @since 2023/11/04 23:38
 */
public class MemoryPropertiesUserManager extends AbstractUserManager {


    public static final String ATTR_WRITE_PERM_PATH = "writepermission.path";
     ...
     */
    public MemoryPropertiesUserManager(PasswordEncryptor passwordEncryptor,
    ...
    public synchronized void save(User usr) throws FtpException {
        ...
        //该行代码为源码中实现。该操作导致了writepermission只有为/时才会生效，如果为/test、/test/test1这种路径，就不会生效了。
        //userDataProp.setProperty(thisPrefix + ATTR_WRITE_PERM, usr.authorize(new WriteRequest()) != null);

        //fix bug: 如果拥有了写权限，则用户的写权限标识符为true
        userDataProp.setProperty(thisPrefix + ATTR_WRITE_PERM, false);
        for (Authority authority : usr.getAuthorities()) {
            if (authority instanceof MemoryWritePermission) {
                MemoryWritePermission a = (MemoryWritePermission) authority;
                userDataProp.setProperty(thisPrefix + ATTR_WRITE_PERM, true);
                userDataProp.setProperty(thisPrefix + ATTR_WRITE_PERM_PATH, a.getPermissionRoot());
                break;
            }
        }
    ...
    public User getUserByName(String userName) {
        ...
        if (userDataProp.getBoolean(baseKey + ATTR_WRITE_PERM, false)) {
            authorities.add(new WritePermission(userDataProp.getString(baseKey + ATTR_WRITE_PERM_PATH, "/")));
        }
